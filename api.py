...
def configure_ngrok():
    ngrok_token = "2pR0umCPyC9vztYhPjNjIESxjGz_5oRhAUhM1XeDwJ282G5MT"
    try:
...
def update_soul_txt(public_url):
    with open("anony6.txt", "w") as file:
        file.write(public_url)
    print(f"New ngrok link saved in anony6.txt")
...
def update_vps_soul_txt(public_url):
    ...
    try:
        ...
        sftp = ssh.open_sftp()
        with sftp.open("anony6.txt", "w") as file:
            file.write(public_url)
        sftp.close()
        ssh.close()
        print("Updated anony6.txt on VPS successfully.")
    except Exception as e:
        print(f"Failed to update anony6.txt on VPS: {str(e)}")
